...
**Authors:** David Dalmazzo, Laura Cros Vila, Luca Casini, Bob L.T. Sturm

> **Note on this round.** The three *Regular Reviews* (Reviewer 1, 2, and 3) below are reproduced **verbatim from Round 2**. Every point in them was addressed in our Round-2 response letter and is present in the current manuscript. We mark each such item as **(duplicated) Point resolved on the previous round.** Genuinely new assessment appears under **Editor Meta-Review** and **Additional Review**, which we answer in full.
...
**Answer:** There is no contradiction once the pipeline is stated clearly, and we have fixed the wording that obscured it. The PARC benchmark limitation applies to *end-to-end* models that predict Roman numerals **directly from audio**, which is not what we do. We run ACE to obtain absolute chord labels and then convert those to Roman numerals **symbolically**, via global key estimation and music21 (Section 4.1). The weakness of direct audio-to-RNA models is therefore the *reason* for our two-step route, not an argument against our own results. We reworded the sentence to make this explicit: it now specifies that it is direct audio-to-RNA models that achieve modest accuracy, and adds that we instead derive Roman numerals symbolically from the ACE chord labels, separating chord recognition from key-relative interpretation.
...
**Answer:** Done. We removed the Jazz observation from the results (Section 5.1) and moved it into the limitations discussion (Section 6.1), rephrased as a labeling limitation: style labels are user-supplied and inconsistent across platforms, and the `Jazz' tag on Suno and Udio largely denotes ambient instrumental music rather than standard jazz harmony, so any per-genre comparison under that label should be read with caution.
...
**Answer:** We ran exactly the test the reviewer demanded. The numbers contradict his prediction. The relative frequency of rotation-equivalent / non-rotation-in-set / out-of-set windows, computed over every four-chord window of the deduplicated Roman numeral sequences, is: Lastfm 1.24% / 2.18% / 96.59%; Suno 6.69% / 5.64% / 87.68%; Udio 1.73% / 2.77% / 95.51%. Among windows built on an axis chord set, the share that is the literal loop (a rotation) is 54.3% for Suno against 36.3% for Lastfm and 38.5% for Udio. Suno is not "using the same four common chords": it plays the loop, in order, about 1.5 times more often than humans. We then applied the strict rotation-only definition the reviewer prefers (12 patterns, zero reordering). Suno's lead over humans does not shrink, it grows at every threshold (Suno-to-Lastfm ratio of songs above the threshold, any-order → loop-only): ≥10% 3.24× → 4.84×; ≥25% 4.54× → 6.26×; ≥50% 8.05× → 12.45×. As an independent check, we recomputed the three-way classification from the released tetragram dataset, a separately constructed artifact; the loop shares match to the decimal.

The manuscript now reports both measures side by side. Section 4.5 defines axis-family membership (72 patterns) and the literal loop (12 rotations) and states the Richards rationale: rotations are musically meaningful, wider reorderings are not. Table 5 reports the per-song rates under both measures, Figure 10 shows both distributions, and Figure 11 gives the loop-versus-reordering breakdown. On the Roman-numeral point: rotation equivalence also absorbs the relative-major/minor ambiguity of the global key estimate. C, G, Am, F is I-V-vi-IV when the key is read as C major and III-bVII-i-bVI, the minor-form set begun on a different chord, when it is read as A minor; treating the three sets and their rotations as one loop recovers the progression under either reading, so a shaky key call cannot delete an axis song. Section 4.5 states this.

---

# Regular Reviews

> Reviewers 1–3 are reproduced verbatim from Round 2. Each point was resolved in our Round-2 response and is in the current manuscript.

---

## Reviewer 1 (Recommendation: Revisions Required)

### 1. Originality: *Highly original*
This study introduces a novel musicological framework for the large-scale comparison of AI-generated and human-composed music. By automating the analysis of chord progressions and musical structure, the authors deliver a highly original, systematic investigation that enriches the literature and establishes a practical benchmark.

*(No action requested.)*

### 2. Previous work: *Medium high*
The related-work section is now comprehensive. A few remaining remarks on organization and factual accuracy:

**Comment.** *Organization of Section 2.3*: the second paragraph feels disjointed, transitioning rapidly across handcrafted→deep-learning methods, paper summaries, open implementations, implementation issues, and Roman Numeral Analysis. Restructure for logical progression.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Factual Clarifications in Section 2.2:*
- MSA generally relies on **four** principles (add *novelty*), not three.
- (Levy and Sandler, 2008) and (Serrà et al., 2012) are not spectral clustering; (McFee and Ellis, 2014) is (Laplacian decomposition).
- (Marmoret et al., 2023) does not use deep learning and belongs in the first paragraph.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Section Organization*: consider separating "Evaluation of AI-Generated Music" and "Harmony Analysis of Large-Scale Corpora" into two subsections (authors' discretion).

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Bibliography proofreading:*
- "Joan Serrà" spelled inconsistently ("Serrà" vs "Serra"); standardize to "Serrà".
- Update arXiv preprints to published venues: Lin et al. 2023 (ICCC 2023); Marmoret et al. 2023 (TISMIR); Park et al. 2019 (ISMIR); Peeters 2023 (ISMIR); Waseem Akran et al. 2025 (IEEE TASLP); Wu et al. 2020 (ISMIR).
- Standardize ISMIR proceedings venue formatting.
- Librosa: use the Zenodo citation.

**Answer:** (duplicated) Point resolved on the previous round. *(The two items the editor found still open, Lin → ICCC 2023 and page numbers for the ISMIR entries, are now corrected in this revision; see the Editor Meta-Review bibliography response above.)*

---

### 3. Methodology: *Sufficient methodology*
**Comment.** One remaining concern on potential selection bias in the dataset (elaborated in "Overall value" below).

**Answer:** Addressed in full under **Overall value** below.

---

### 4. Clarity: *Somewhat clear*
**Comment.** Background (Section 2) would benefit from refinement; Section 3 (Corpora) could organize the three datasets into AI-generated vs human-composed subsections.
...
### 5. Reproducibility
Code, metadata, and estimated features released open-source; final GitHub repo expected in the published version. *(No action requested.)*

### 6. Overall value: *High*
**Comment.** Substantive concern on selection bias in data collection. In the original manuscript, Suno was sourced from the "New Songs" playlist and Udio by popularity (most plays within a 24-hour window). In the revision the Udio popularity detail was removed; the shift is unexplained and concerning. Even with playlist/front-page scraping, opaque platforms make it impossible to guarantee equivalent curation: if Suno is recency-driven and Udio is engagement-curated, the datasets are shaped by two divergent filters. A popularity filter is a proxy for human preference and a confound for the Suno-vs-Udio comparison; Udio's apparent diversity might reflect human curation rather than model capability. Advises addressing this as a limitation and nuancing the comparative conclusions.
...
**Comment.** *Beat-Level Chord Annotations*: mentioned in Abstract/Introduction/Section 8 but absent from the main text; state in the methodology that chords are beat-aligned.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Missing Reference*: on line 109, a citation for the "Jazz Transformer" is missing.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Consistent Terminology (ACE Model)*: the model is named multiple ways (consonance-ACE, consonance-aware ACE model, Conformer-based model); pick one.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Trigram Definition Inconsistency*: Section 4.3 (line 366) includes repetitions; Section 4.4 (lines 395–396) does not. Clarify and standardize.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Discussion of Minor Triads*: minor triads are omitted or mentioned late despite being the second most frequent; mention earlier.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Genre Redundancy*: "Blues" appears twice on lines 511–512; clarify if one means "Rhythm and Blues".

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Figure Consolidation (Figures 1, 2, 3)*: combine into a single figure with three subplots (subcaption package).

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Figure Placement*: from Figure 4 on, figures/tables appear much later than their in-text citations.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Unformatted Subtitles*: Section 5.3 has unformatted subtitles ("Form Diversity and Style Differences", "Summary").

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Structural Patterns in Figure 7*: the second most frequent structural pattern in both Suno and Udio never appears in the human dataset; warrants a brief comment.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Irrelevant Algorithm Mention*: line 686 "Laplacian Segmentation" feels out of place.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Udio Track Lengths*: many Udio songs last under a minute; add a brief explanation.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** *Broader Impact Statement*: the conclusion could more strongly emphasize the automated pipeline as a methodological advancement.

**Answer:** (duplicated) Point resolved on the previous round.

---

### 7. Ethical approval: N/A
### 8. Other Comments: *(none)*

---

## Reviewer 2 (Recommendation: Revisions Required)

### 1. Originality: *Somewhat original*
Compares AI-generated and human-composed pop music from structure and harmony; shows Suno uses canonic progressions and simple structures while Udio resembles human-composed patterns. Timely musicological investigation. *(No action requested.)*

### 2. Previous work: *High*
Necessary related work discussed; based on the current frontier of MIR techniques. *(No action requested.)*

### 3. Methodology: *Sufficient methodology*
General praise, with room for improvement (harmonic rhythm not considered; progressions beyond four chords excluded, e.g., the canon progression). Further questions:

**Comment.** Confusion about when absolute chord labels are converted to Roman numerals: Sections 4.1/4.5 say right after ACE prediction (then consecutive labels merged), while 4.3 says trigrams are generated first then converted. If consecutive RNs are merged, some trigrams could be reduced.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Table 4: how are 0%, 1%, 5% chosen? A song using one progression for only 1–5% seems uncommon; suspects ACE noise. Requests Section 5.4 be discussed in more detail and the limits noted in 6.1.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Section 4.4 says structure analysis uses each section label as a token, but Figure 7 uses trigrams; result section does not refer to the methodology section.

**Answer:** (duplicated) Point resolved on the previous round.

---

### 4. Clarity: *Somewhat unclear*
Main message clear; results analysis hard to follow / unstructured, disconnecting conclusions from results.

**Comment.** Section 2.1 mixes AI Music Generation and Harmonic Analysis; "large-scale" in 2.1.2 overclaims (cited works are corpus-scale, human-manageable); "corpus analysis" is more accurate.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Discussion of results lacks detail (e.g., Section 5.1 / Figures 1–3): observations from the figures are not described beyond genre tendencies and "similar" distributions. Missing conclusive messages (e.g., six most frequent trigrams are permutations of I, IV, V; vi appears in Figure 2's second most frequent trigram).

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Section 5.2 starts by referring to Figure 8 without describing it; unclear how Figure 8 is generated; canonical progressions appear only in 4.5, and it discusses p-trigrams vs trigrams.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Section 5.3 results (Figure 7) are not referred to in the text; under-explained until 6.3.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Line 592: the "Unique Tokens Ratio" results are missing.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Prefers a "what the figure shows → observations → conclusions" structure; observations are missing, disconnecting messages from results.
...
### 5. Reproducibility
Good; code available, datasets public. *(No action requested.)*

### 6. Overall value: *Medium high*
Much improved; ready to publish modulo presentation. Minor comments:

**Comment.** Lines 28–32: song numbers too specific for the introduction; maybe move to Section 3.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Line 235: "impractical" understates; it just requires more effort.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Lines 279–281 describe Suno data collection but not Udio.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Line 311: D:m7 should be D:min7.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Line 317: maybe add a citation.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Lines 344–349: show the example output as Table 2 (shorten Table 2 if needed).

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Use consistent formatting for chord labels (typewriter vs italic vs plain).

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Lines 389 & 400: why 500/150 merge operations?

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Line 404: six chords or four? (Later sections suggest four.)

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Figure 2: y-axis ticks repeating.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Figures 1–3 easier to compare on the same page.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Lines 490–496: sharp 9th / flat 13th not in Figure 6; ACE assumes enharmonic equivalence and encodes them as flat 3rd/5th/6th; mention in limitations.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Figure 5: usefulness unclear given Roman-numeral analysis; estimated-key distribution might be more informative. Lines 591/603: are they subsubsections?

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Line 598 seems based on the previous version (non-semantic structure labels).

**Answer:** (duplicated) Point resolved on the previous round.

---

### 7. Ethical approval: Not needed (existing datasets)
### 8. Other Comments: *(none)*

---

## Reviewer 3 (Recommendation: Revisions Required)

### 1. Originality: *Somewhat original*
Timely, meaningful contribution applying large-scale MIR to musicological questions; corpus of 60,014 tracks; Axis of Awesome analysis and semantic section labels strengthen originality. Broad conclusions are confirmatory (empirical substantiation of anecdotal knowledge). *(No action requested.)*

### 2. Previous work: *Medium high*
Literature review substantially improved; Richards (2017) and Biamonte (2010) well chosen.

**Comment.** Genre-ontology concern remains underexplored: 'Jazz' on Suno/Udio differs from Last.fm; the per-genre spider charts (Figure 6, Figure 9) deserve a sentence of caution: comparing the 'Jazz' polygon across collections compares different musical objects.
...
### 3. Methodology: *Sufficient methodology*
Significant improvement (consonance-ACE; All-In-One). Two observations:

**Comment.** Axis of Awesome (4.5) treats all 24 orderings as valid (72 patterns); chord order is consequential. As defined, axis rates measure axis-family vocabulary membership more than recurrence of a single progression. Note this distinction explicitly.

**Answer:** (duplicated) Point resolved on the previous round. *(See also the Additional Review §4.5 answer above: the manuscript now reports both measures, any order and strict rotation-only, and the strict measure strengthens the finding.)*

---

**Comment.** The revised BPE analysis of semantic section labels is a substantial improvement; the key limitation (acoustically assigned labels; verse–chorus alternation may reflect loop contrast rather than functional form) is clearly stated (6.3). *(Positive; no action requested.)*

---

### 4. Clarity: *Somewhat clear*
Better structured (RQ1–RQ3 spine). Two issues:

**Comment.** Add a brief orienting paragraph separating what each metric captures (chord-quality distributions; trigram frequencies/correlations; axis rates; longer recurring patterns). The Udio–Last.fm trigram correlation (r = 0.952) reflects distributional similarity, not equivalence of formal position or rate of harmonic change.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Hedging around 'algorithmic monoculture' (6.4) is improved but still evaluatively loaded ("exemplifies this pattern"; "production of digital files, as a gamification process"). Acknowledge that harmonic restriction is not inherently a compositional deficiency (twelve-bar blues, punk, minimal techno, chill-out).
...
### 5. Reproducibility
Clear strength (MIT-licensed annotations, boundaries, metadata, code; unmodified pretrained models). Proprietary-platform churn acknowledged (Suno ≤ v3.5). *(No action requested.)*

### 6. Overall value: *Medium high*
Substantial, conscientious reworking. Remaining focused revisions:

**Comment.** (1) A sentence on the all-orderings assumption in the Axis analysis. (2) A brief acknowledgement that harmonic restriction is not inherently a deficiency. (3) A brief orienting paragraph distinguishing what the harmonic metrics capture.
...
### 7. Ethical approval
No human subjects; data-acquisition ethics addressed in Section 9 (EU Copyright Directive TDM exception). *(No action requested.)*

### 8. Other Comments

**Comment.** Suno/Udio 'Jazz' lacking standard jazz harmony deserves more attention; it may reflect user prompts ('chill', 'relaxing') rather than model capacity; user prompts are a confound. A sentence noting this ambiguity would be valuable.

**Answer:** (duplicated) Point resolved on the previous round.

---

**Comment.** Vocabulary counts (Suno 6,770 unique trigram types vs Udio 8,056 vs Last.fm 9,838) should be interpreted cautiously: Last.fm spans decades/genres/cultures, Suno is one model over a limited period; flag the asymmetry.
